...
def solve_model(system, solver='glpk'):
...
def write_results_summary(system, folder):
    import DOOM.classes
    demands = {}
    supplies = {}
    other_units = {}
    ...
    for key, val in system.unit.items():
        ...
        if isinstance(val, DOOM.classes.Demand) or isinstance(val, DOOM.classes.HeatDemand):
            demands.update({key: val})
        elif isinstance(val, DOOM.classes.Supply):
            supplies.update({key: val})
        elif not isinstance(val, DOOM.classes.Coupler):
    ...
    with open(folder+'/results_summary.txt', 'w') as f:
        ...
        for name, dem in demands.items():
            ...
            if isinstance(dem, DOOM.classes.HeatDemand):
...
def write_full_results(system, folder):
    ...
    with open(Path(Path.cwd() / folder / 'full_results.txt'), 'w') as f:
        ...
        for name, unit in system.unit.items():
            ...
            try:
                for seq in unit.var['seq']:
                    vals = list(unit.var['seq'][seq].get_values().values())
                    f.write("\n" + seq +
                            ": Maximum {}, Minimum {}".format(
                                max(vals),
                                min(vals)
                            ))
                    with open(Path.joinpath(Path.cwd(), folder, 'timeseries', name+"_"+seq+".txt"), 'w') as ts:
